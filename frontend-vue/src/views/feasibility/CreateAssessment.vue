    ...
    >
      ...
      <el-form-item label="需求描述" prop="rawRequirement">
        <el-input
          ...
          type="textarea"
          :rows="8"
          placeholder="请详细描述您的检测需求，包括检测对象、场景、要求等"
          maxlength="2000"
          show-word-limit
        />
      </el-form-item>
...
const handleUploadSuccess = (response, file) => {
  if (response.success && response.data) {
    form.imageUrls.push(response.data.url || response.data.path)
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await feasibilityAPI.createAssessment({
      assessmentName: form.assessmentName,
      rawRequirement: form.rawRequirement,
      imageUrls: form.imageUrls
    })

    ElMessage.success('创建成功')
    router.push(`/feasibility/${res.data.id}`)
  } catch (error) {
    if (error !== false) {
