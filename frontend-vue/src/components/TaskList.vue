  ...
  <div class="task-list">
    <div class="toolbar">
      ...
      >
        ...
        <el-option label="DINO 检测" value="DINO_DETECTION" />
        <el-option label="VLM 清洗" value="VLM_CLEANING" />
...
const loadTasks = async () => {
  ...
  try {
    ...
    const response = await algorithmAPI.getTasks(params)
...
const cancelTask = async (task) => {
  try {
    ...
    let cancelAPI
    if (task.taskType === 'DINO_DETECTION') {
      cancelAPI = algorithmAPI.cancelDinoTask(task.taskId)
    } else if (task.taskType === 'VLM_CLEANING') {
      cancelAPI = algorithmAPI.cancelVlmTask(task.taskId)
    } else if (task.taskType === 'YOLO_DETECTION') {
      cancelAPI = algorithmAPI.cancelYoloTask(task.taskId)
    }
...
const downloadResults = async (task) => {
  try {
    let resultsAPI
    if (task.taskType === 'DINO_DETECTION') {
      resultsAPI = algorithmAPI.getDinoTaskResults(task.taskId)
    } else if (task.taskType === 'VLM_CLEANING') {
      resultsAPI = algorithmAPI.getVlmTaskResults(task.taskId)
    } else if (task.taskType === 'YOLO_DETECTION') {
      resultsAPI = algorithmAPI.getYoloTaskResults(task.taskId)
    }

    const response = await resultsAPI
    ...
    link.href = url
    link.download = `${task.taskId}_results.json`
    link.click()
