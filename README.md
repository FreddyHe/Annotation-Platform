...
请先阅读 **[非 Docker 部署与模型准备指南](doc/DEPLOYMENT.md)**。
本仓库包含平台前后端、模型适配器与标注界面定制代码，**不包含模型权重、上传数据、数据库或账号凭据**。
基础预标注需要自行准备 GroundingDINO + BERT；完整智能标注还需要 LocateAnything-3B，语义复核需要 Qwen3-VL-4B-Instruct。训练使用自己的 YOLO 权重。
具体来源、文件结构、环境版本与逐服务启动方式见上述指南；本文下方为既有功能说明。
