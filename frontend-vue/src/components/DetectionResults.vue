  ...
  <div class="detection-results">
    <div class="toolbar">
      ...
      <el-input-number
        ...
        @change="loadResults"
      />
      <span style="margin-left: 10px;">最小置信度</span>
    </div>
    ...
    <el-table :data="filteredResults" v-loading="loading" style="width: 100%">
      ...
      <el-table-column prop="bbox" label="边界框" width="300">
        <template #default="{ row }">
          <span>[{{ row.bbox.map(v => v.toFixed(2)).join(', ') }}]</span>
        </template>
      </el-table-column>
        ...
        <div class="preview-info">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="标签">
              <el-tag type="primary">{{ currentResult.label }}</el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="置信度">
              {{ (currentResult.confidence * 100).toFixed(2) }}%
            </el-descriptions-item>
            <el-descriptions-item label="边界框">
              [{{ currentResult.bbox.map(v => v.toFixed(2)).join(', ') }}]
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
...
const availableLabels = computed(() => {
  ...
  results.value.forEach(r => labels.add(r.label))
...
const filteredResults = computed(() => {
  ...
  if (filterLabel.value) {
    filtered = filtered.filter(r => r.label === filterLabel.value)
  }

  if (minConfidence.value > 0) {
    filtered = filtered.filter(r => r.confidence >= minConfidence.value)
  }
